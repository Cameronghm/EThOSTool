...
function send_new_ticket(search_input, number_of_groups, clustering_dimensions, summary_type) {
    var xmlHttp = new XMLHttpRequest();
    xmlHttp.onreadystatechange = function () {
        if (xmlHttp.readyState === 4 && xmlHttp.status === 200)
    ...
    FD.append("summary_type", summary_type);
...
function submit_ticket() {
    ...
    var category = document.getElementById("dtype");
    var dtype = parseInt(category.value);
    var sumtype = document.getElementById("stype");
    var stype = parseInt(sumtype.value);

    if (dtype != 19)
    {
        if (stype != 0)
        {
            var summary_type = stype;
        }
        else
        {
            var summary_type = summary_conversion[dtype];
        }
    }
    else {
        var summary_type = 2;
    ...
    send_new_ticket(txt, $('#group_number_range').val(), clustering_dimensions, summary_type);
...
function create_ticket_HTML(ticket_id, ticket_name, ticket_progress, ticket_status, ticket_priority, summary_type) {
...
function ticket_queue_response(xmlHttp_response) {
    ...
    if (json_response.length > 0) {
        for (var i = 0; i < json_response.length; i++) {
            ...
            var current_ticket_summary_type = saved_ticket_data[i].summary_type;

            var ticket_HTML = create_ticket_HTML(current_ticket_id, current_ticket_name, current_ticket_progress, current_ticket_status, current_ticket_priority, current_ticket_summary_type);
