...
def search():
    ...
    if request.method == 'POST':
        ...
        if message_tag == "all_papers_in_ticket":
            ...
            result = submit_ticket(request.form['search_terms'], int(request.form['group_size']),int(request.form['clustering_dimensions']),int(request.form['summary_type']))
...
def all_papers_in_ticket(ticket_id):
    ...
    all_papers = []
    for group in current_ticket.papers_in_each_group.items():
        all_papers = all_papers + group[1]

    return create_json_from_papers(all_papers, ticket_id)

def submit_ticket(searched_text, group_size, clustering_dimensions, summary_type):
    """Simply submits the searched text as a new ticket, then returns a JSON description of the queue"""
    global group_amount

    ticketmanager.create_new_ticket(searched_text, group_size, clustering_dimensions, summary_type)
