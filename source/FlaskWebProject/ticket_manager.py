...
tickets = {}
biggest_ticket_id = -1
TICKETS_PATH = 'FlaskWebProject\\database\\tickets\\'
...
class Ticket:
    ...
    def __init__(self, _ID, _search, _summary_type):
        self.ID = _ID
        self.search = _search
        self.papers_and_ids = {}
        ...
        self.progress = 0
        self.summary_type = _summary_type
...
def process_ticket(ticket):
    ...
    for pdf in ticket.papers_and_ids:
        open(backend.SUMMARY_TYPE_PATH+pdf+'.txt', "w").write(str(ticket.summary_type))
...
def create_new_ticket(searched_text, group_size, clustering_dimensions, summary_type):
    """
    NOT COMPLETE!
    ...
    tickets[ticket_id] = Ticket(ticket_id, searched_text, summary_type)
...
def get_ticket_queue():
    ...
    for i, ticket_key in enumerate(tickets):
        ...
        summary_type = tickets[ticket_key].summary_type
        queue_pos = i
        ticket_list.append({"ticket_id": ticket_id, "ticket_name": ticket_name, "progress":progress, "status":status, "priority":priority, "queue_pos":queue_pos, "summary_type": summary_type})
