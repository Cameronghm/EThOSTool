...
import pdfplumber
import re
import FlaskWebProject.summarisers as summarisers
import fitz
...
SUMMARY_TYPE_PATH = 'FlaskWebProject/database/summary_types/'
...
def check_folders():
    global PDF_INPUT_PATH,SUMMARISED_PATH,RAW_TEXT_PATH,THUMBS_PATH,PREVIEWS_PATH,WORD_VECTOR_PATH,DOWNLOADS_PATH, SUMMARY_TYPE_PATH 
    folders = [PDF_INPUT_PATH,SUMMARISED_PATH,RAW_TEXT_PATH,THUMBS_PATH,PREVIEWS_PATH,WORD_VECTOR_PATH,DOWNLOADS_PATH, SUMMARY_TYPE_PATH ]
    for f in folders:
...
def SummarisePaper(paper_name):
    global stop_word, RAW_TEXT_PATH, PDF_INPUT_PATH,changed_paper_names, paper_locations, SUMMARY_TYPE_PATH 
    print("Summarising: " + paper_name)

    type_dictionary = {
        1: "legacy", 
        2: "allenai/led-base-16384",
        3: "allenai/led-large-16384",
        4: "allenai/led-large-16384-arxiv",
        5: "patrickvonplaten/led-large-16384-pubmed",
        6: "MingZhong/DialogLED-base-16384",
        7: "pszemraj/led-base-book-summary",
        8: "pszemraj/led-large-book-summary",
        9: "nsi319/legal-led-base-16384"}

    input = ""

    input = get_abstract(paper_name)

    if (input == "Abstract Not Found"):
        return "Abstract Not Found"

    summary_type = int(open(SUMMARY_TYPE_PATH + paper_name + '.txt','r').read())
    ...
    #    print(e)
    #    return None
    if type_dictionary[summary_type] == "legacy":
        if input:
            summarization = ""
        ...
        if len(input) > 0:
            summarization = summarisers.new_summariser(input, type_dictionary[summary_type])
            summarization = summarization.replace('</s>', '')
            summarization = summarization.replace('<s>', '')
...
def clean_files(all_paper_names):
    global SUMMARISED_PATH,RAW_TEXT_PATH,THUMBS_PATH,PREVIEWS_PATH,WORD_VECTOR_PATH, SUMMARY_TYPE_PATH 
    folders = [SUMMARISED_PATH,RAW_TEXT_PATH,THUMBS_PATH,PREVIEWS_PATH,WORD_VECTOR_PATH, SUMMARY_TYPE_PATH ]
    for f_path in folders:

        filelist = [ f for f in os.listdir(f_path) if f.endswith(".txt") or  f.endswith(".png")]
...
def refresh_summarisation_queue():
    global all_paper_names, paper_queue, RAW_TEXT_PATH, SUMMARISED_PATH, summary_type
...
def complete_summarisation_queue():
    global all_paper_names, paper_queue, stop_word, stop_word_set, summarised_papers, SUMMARISED_PATH, summary_queue
    ...
    if len(paper_queue) > 0:
        ...
        while len(paper_queue) > 0:
            ...
            f = open(SUMMARISED_PATH + current_paper_name + ".txt", "w", errors='ignore') #, encoding="utf8"
            f.write(result)
            f.close()
...
def update_database(rebuild_summarisations):
    # OLD PROCESS
    global processes_total, processes_complete, current_process_status,word_vector_names_order, word_vector_matrix, all_paper_names, summary_type
...
def read_and_store_pdf(paper_name):
    ...
    if len(out_str) < 5:
        ...
        f = open(RAW_TEXT_PATH + paper_name + ".txt", "w", errors='ignore')  #, encoding="utf8"
        f.write(out_str)
        f.close()
...
def findTitle(paper_name):
    ...
    if path.exists(test_path):
        try:
            f = open(test_path, "r", errors='ignore')
            out = f.read()
            f.close()
            return out + "\n"
...
def write_paper_preview(paper_name, start_line = 0):
    ...
    f = open(RAW_TEXT_PATH + paper_name + ".txt", "r", errors='ignore')
...
def create_word_vectors():
    ...
    with open(NAMES_ORDER_PATH, 'w', errors='ignore') as f:
...
def load_existing_word_vectors():
    ...
    if path.exists(NAMES_ORDER_PATH):
        with open(NAMES_ORDER_PATH, 'r', errors='ignore') as f:
...
def get_summarised_text(paper_name):
    global summarised_papers, SUMMARISED_PATH, summary_type
    if paper_name in summarised_papers:
        return summarised_papers[paper_name]
    else:
        try:
            f = open(SUMMARISED_PATH + paper_name + ".txt", "r", errors='ignore') #, encoding="utf8"
...
def get_thumbnail_url(paper_name):
    ...
    thumb_path = THUMBS_PATH + paper_name + '.png'
    if path.exists(thumb_path):
        return thumb_path
    else:
        try:
            fitzopen = fitz.open(paper_locations[paper_name])
            fitzopen[0].get_pixmap(dpi=25).save(thumb_path)
            return thumb_path
        except:
...
def get_paper_preview(paper_name):
    ...
    if paper_name in paper_previews:
        ...
        if not os.path.exists(PREVIEWS_PATH + paper_name + ".txt"):
            ...
            f = open(PREVIEWS_PATH + paper_name + ".txt", "r", errors='ignore') #, encoding="utf8"
...
def get_abstract(paper_name):
    pdf_path = get_paper_pdf_url(paper_name)
    ...
    for page in range(30):
        ...
        text = text.replace("\n", " ")
        text = text.replace("  ", " ")
        retext = "Abstract.{0,1}([A-Z]|[a-z]|-{0,1}[0-9]+| |\n|\t|!|-|,){20,}|ABSTRACT.{0,1}([A-Z]|[a-z]|-{0,1}[0-9]+| |\n|\t|!|-|,){20,}"
        condition = re.search(retext, text)
        if condition:
            if ("Abstract" in text):
                text = text[text.index("Abstract")+8:]
            elif ("ABSTRACT" in text):
                text = text[text.index("ABSTRACT")+8:]
            finish_check = text.split(".")
            #print(finish_check)
            #Check to see if there is a full-stop after the last sentence
            if (re.search("([A-Z]|[a-z]| |\n|\t|!){20,}", finish_check[-1]) != None):
    ...
    retext = "\.{2,}"
    splits = re.split(retext, abstract)
    while len(splits) > 1:
        if len(splits[0]) < len(splits[1]):
            abstract = abstract[abstract.index(splits[1]):]
        else:
            abstract = abstract[:abstract.index(splits[1])]
        splits = re.split(retext, abstract)
    ...
    while len(splits) > 1:
        ...
        splits = re.split(retext, abstract)
    abstract = abstract.replace(". .","")
    retext = "acknowledgements|Acknowledgements"
    has = re.search(retext, abstract[len(abstract)//3:])
    if (has):
        abstract = abstract[:has.start()]
    print(abstract)
    if len(abstract) < 300:
        return "Abstract Not Found"
...
def get_raw_text(paper_name):
    ...
    with open(RAW_TEXT_PATH + paper_name + ".txt",'r', errors='ignore') as f:
        text = f.read()
    return text
